...
import { INPUT_ENDERECO } from "@/components/user/constants"
  ...
  return (
    <section className={cn(style.section.mobileFirst, style.section.sm)}>
      <GroupInSection variant="colsOne">
        <Titulo variant="bold">Seu endereço</Titulo>
        {Object.keys(data.entrega).length > 0 ?
          <ScrollSection>
            <DataIlustrator>
              <MapComponent names={Object.keys(data.entrega)}>
                  {keyName => <Span>{keyName}: {data.entrega[keyName]}</Span>}
              </MapComponent>
            </DataIlustrator>
          </ScrollSection>
          :
          <section className="w-full mb-[15vw] sm:mb-[5vw]">
            <p className="font-Poppins_light text-[4vw] sm:text-[1vw] text-center sm:text-left w-8/12 mx-auto sm:mx-0 mb-[10vw] sm:mb-[2vw]">Você não possui <strong className="text-[#8c52ff]">nenhum endereço salvo!</strong></p>
          </section>
        }

        <Titulo variant="bold">Endereço de entrega</Titulo>
        <Form>
          <MapComponent names={INPUT_ENDERECO}>
